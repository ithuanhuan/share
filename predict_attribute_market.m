%加载训练好的模型，预测训练集的各个属性概率并且写入文件
%clear;
netStruct = load('./data/resnet_market/net-epoch-55.mat');
net = dagnn.DagNN.loadobj(netStruct.net);
net.move('cpu');
net.mode = 'test' ;
net.conserveMemory = false;
im_mean = net.meta(1).normalization.averageImage;
% p = dir('/Users/huan/Downloads/re-id/dataset/Market1501/bounding_box_test/*.jpg');
p = dir('./Market1501/bounding_box_test/*.jpg');
%todo 修改13115为20
uu = zeros(1,13115);
dd = zeros(1,13115);
...
%for j = 6618:numel(p)
    %i = j-6617;
fid=fopen('test_attr.txt','wt'); %写的方式打开文件（若不存在，建立文件）；
for j = 1 : numel(p)
    i = j
    str = strcat('/Users/huan/Downloads/re-id/dataset/Market1501/bounding_box_test/',p(j).name);
    %todo 
