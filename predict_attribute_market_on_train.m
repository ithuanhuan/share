%加载训练好的模型，预测训练集的各个属性的概率并且写入文件
clear;
netStruct = load('/Users/huan/code/matlab/share/data/resnet_market/net-epoch-55.mat');
net = dagnn.DagNN.loadobj(netStruct.net);
net.move('cpu');
net.mode = 'test' ;
net.conserveMemory = false;
im_mean = net.meta(1).normalization.averageImage;
p = dir('/Users/huan/Downloads/re-id/dataset/Market1501/bounding_box_train/*.jpg');
%todo 修改13115为10
uu = zeros(1,12936);
dd = zeros(1,12936);
gallery = zeros(12936,28);
%uu = zeros(1,10);
%dd = zeros(1,10);
%gallery = zeros(10,28);
...
%for j = 6618:numel(p)
    %i = j-6617;
fid=fopen('train_attr.txt','wt'); %写的方式打开文件（若不存在，建立文件）；
for j = 1 : numel(p)
    i = j
    str = strcat('/Users/huan/Downloads/re-id/dataset/Market1501/bounding_box_train/',p(j).name);
    %todo 
    ...
    [maxd,dd]=max(d);
    gallery(i,11) = uu;
    %往数组里面添加元素，第28个，其实是从27个里面的8个里面取的最大值
    a(end+1) = uu
    gallery(i,12) = dd;
    %往数组里面添加元素，第29个，其实是从27个里面的9个里面取的最大值
    a(end+1) = dd
    fprintf(fid,'%s',str);  
    fprintf(fid,' ');
    fprintf(fid, '%f ', a)
